...
const updateItemStatus = async (req, res) => {
    try {
        ...
        const item = order.orderItems.find(item => item._id.toString() === itemId);//converting the id to string that matches item id passed in body
        if (!item) {
            return res.status(404).json({ 
                success: false, 
        ...
        if (status === 'Cancelled' && order.paymentMethod !== 'COD') {
            ...
            wallet.transactions.push({
                ...
                status: 'completed',
                purpose: remainingNonCancelledItems.length === 0 ? 'cancellation_order_refund' : 'return_order_refund',
        ...
        if (allDelivered) {
            ...
            order.finalAmount -= (remainingNonCancelledItems.length === 0 ? item.price * item.quantity + order.deliveryCharge : item.price * item.quantity);
        }
